...
class FitDatasetReducedChi(AbstractFit):
    # noinspection PyUnresolvedReferences
    def __init__(
        self,
        dataset,
    ...
    def log_likelihood_with_regularization(self) -> float:
        ...
        if self.inversion is not None:
            return fit_util.log_likelihood_with_regularization_from(
                chi_squared=self.reduced_chi_squared,
    ...
    def log_evidence(self) -> float:
        ...
        if self.inversion is not None:
            return fit_util.log_evidence_from(
                chi_squared=self.reduced_chi_squared,
                regularization_term=0.0,
                log_curvature_regularization_term=0.0,
    ...
    # @property
    @cached_property
    def reduced_factor(self) -> int:
        return int(np.size(self.mask) - np.sum(self.mask))
    @property
    def reduced_chi_squared(self) -> float:
        return self.chi_squared / self.reduced_factor
